...
public class AlbumController {
	...
	public static final  String RECENT_PHOTO = UIUtils.getString(R.string.最近照片);
	...
	public List<String> getCurrent() {
		...
		Cursor cursor = resolver.query(Media.EXTERNAL_CONTENT_URI, new String[] {ImageColumns.DATA,
                                       ...
                                       ImageColumns.SIZE+" > "+IMAGE_SIZE , null, ImageColumns.DATE_ADDED+"  DESC");
		if (cursor == null) {
			return photos;
		}
	...
	public List<AlbumModel> getAlbums() {
		...
		if (cursor == null || !cursor.moveToNext())
			return albums;
		cursor.moveToLast();
		AlbumModel current = new AlbumModel(RECENT_PHOTO, 0, cursor.getString(cursor.getColumnIndex(
                ImageColumns.DATA))); // "最近照片"相册
		albums.add(current);
		...
		do {
			...
			if (map.keySet().contains(name))
				...
				AlbumModel album = new AlbumModel(name, 1, cursor.getString(cursor.getColumnIndex(
                        ImageColumns.DATA)));
				map.put(name, album);
				albums.add(album);
			}
