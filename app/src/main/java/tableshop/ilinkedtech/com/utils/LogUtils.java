...
public class LogUtils {
     ...
     * 日志输出时的TAG
     */
    private static      String mTag      = Const.DEBUG_TAG;
    /**
     * 日志输出级别NONE
     */
     ...
     * 是否允许输出log
     */
    private static int mDebuggable = LEVEL_ALL;//TODO LEVEL_OFF LEVEL_ALL
        ...
        if (mDebuggable >= LEVEL_ERROR) {
            if (msg.length()>4000) {
                for (int i = 0; i < msg.length(); i += 4000) {
                    if (i + 4000 < msg.length()) Log.e(tag + i, msg.substring(i, i + 4000));
                    else Log.e(tag + i, msg.substring(i, msg.length()));
                }
            }else {
                Log.e(tag, msg);
            }
