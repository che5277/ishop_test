        ...
        extends ImageView
{
    ...
    private static final float DEFAULT_MAX_SCALE = 2.0f;//TODO 5.0f
    ...
    private boolean mIsRotateEnabled = false;//TODO 是否可以旋转
    /**
     * 需要自动旋转的角度
     */
        ...
        public boolean onTouch(View v, MotionEvent event) {
            if (!mIsTranslate && !mIsRotate && !mIsScale) {
                if (mNormalizedScale == 1) {
                    ...
                    //TODO  通知其父控件，现在进行的是本控件的操作，不允许拦截,自己消费触摸事件
    ...
    private class Scale implements Runnable {
        ...
        private PointF transformCoordBitmapToTouch(float bx, float by) {
            float[] m = getMatrixValues(mMatrix);
            //todo #106-1  NullPointerException
            if (getDrawable()==null){
